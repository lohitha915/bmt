...
$seats=$_POST['seats'];
$price=$_POST['price'];
$sql="INSERT INTO `admtrain` (`sl_no`,`trainno`, `train_name`, `departure`, `destination`, `ddate`,`dtime`,`seats`,`price`) VALUES ('','$trainno', '$train_name', '$departure', '$destination', '$ddate','$dtime','$seats','$price')";
echo $sql;
...
<div class="row  row-cols-md-1 row-cols-lg-2">
  ...
  <p class="pt-2 px-2 pt-md-5 px-md-5">Planning a trip is an exciting part of your new upcoming travel adventure.It’s not for everyone as some swear by only booking a train ticket. It is a journey through time, with the old-fashioned charm and modern amenities.
  </p>
</div>
  <form method="post" action="admtrain.php">
    <div id="form" class="form m-auto m-md-5 p-md-5 bg-dark">
      <h1 class="display-6 text-center text-white">
        <div class="text2">Train</div>
      </h1>
      <div class="row mb-3 text-white">
        <label for="trainno" class="col-sm-2 col-form-label" placeholder="trainno">
          <div class="text1">Train No</div>
        </label>
        <div class="col-sm-10">
          <input type="number" name='trainno' class="form-control" id="trainno" required>
        </div>
      </div>
      <div class="row mb-3 text-white">
        <label for="trainname" class="col-sm-2 col-form-label" placeholder="trainname" required>
          <div class="text1">Train Name</div>
        </label>
        <div class="col-sm-10">
          <input type="text" name='train_name' class="form-control" id="trainname" required>
        </div>
      </div>
      ...
      <button type="submit" class="btn btn-primary" name='addtrain'>Add Train</button></div>
