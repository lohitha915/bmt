...
$updateid=$_GET['id'];
$sql1="SELECT * FROM `admin` WHERE sl_no=$updateid";
...
$seats=$_POST['seats'];
$price=$_POST['price'];
$sql="UPDATE `admin` set `flno`='$flno', `flight_name`='$flight_name', `departure`='$departure' ,`destination`='$destination',`ddate`='$ddate',`dtime`='$dtime',`seats`='$seats',`price`='$price'  where `sl_no`='$updateid'";
  ...
  <form method="post">
    <div id="form" class="form m-auto m-md-5 p-md-5 bg-dark">
      ...
      <div class="row mb-3 text-white">
        ...
        <div class="col-sm-10">
          <input type="number" name='flno' class="form-control" id="flno" value="<?php echo $flno?>" required>
        </div>
      </div>
      <div class="row mb-3 text-white">
        ...
        <div class="col-sm-10">
          <input type="text" name='flight_name' class="form-control" id="flname" value="<?php echo $flight_name?>"required>
        </div>
      </div>
      ...
      <button type="submit" class="btn btn-primary" name='updateflight'>Update Flights</button>
